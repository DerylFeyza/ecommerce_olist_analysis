...
st.markdown(
    """
**Pertanyaan Bisnis:** \n
1. Bagaimana pengaruh harga produk dengan volume penjualan produk pada tahun 2017?
2. Wilayah mana yang melakukan pembelian terbanyak dalam tahun 2017?
"""
)
...
st.subheader("1. Pengaruh Harga Produk dengan Volume Penjualan Produk pada Tahun 2017")
...
combined_2017_df.loc[:, "price_bin"] = pd.cut(
    combined_2017_df["price"], bins=bins, labels=labels, right=False
)
combined_2017_sales_df = (
    combined_2017_df.groupby("price_bin", observed=False)
...
sns.barplot(
    x="price_bin", y="order_item_id", data=combined_2017_sales_df, palette="Blues_r"
)
...
st.subheader("2. Wilayah mana yang melakukan pembelian terbanyak dalam tahun 2017?")
...
combined_df = combined_df.merge(
    br_info, how="left", left_on="customer_state", right_on="sigla"
)
